...
export default function Profile() {
  const [candidateProfile, setCandidateProfile] = useState<any>(false);
  const { userData } = React.useContext(UserContext);
  const { data: profileData, isSuccess: isProfileReturnSuccess } =
    useGetProfileQuery({ skip: true });
  const { data: profileDataCandidate, isSuccess } = useGetProfileByIdQuery({
    id: userData?.id,
  });
  const [deleteProfileSkill, deleteProfileSkillResponse] =
    useDeleteProfileSkillMutation();
  const [putProfile, putProfileResponse] = useUpdateProfileMutation();
  const { data: skillResp, isSuccess: isSkillsReturnedSuccess } =
    useGetSkillsQuery({
      page_number: 0,
      page_size: 0,
    });
  ...
  const [skillsCandidate, setSkillsCandidate] = useState<skillDto[]>([]);
  const [experiences, setExperiences] = useState(
    JSON.parse(localStorage.getItem("constants")!)?.experiences
  );
  ...
  useEffect(() => {
    if (isSuccess) {
      console.log("~", profileDataCandidate);
    }
  }, [isSuccess]);
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      if (profileDataCandidate.messages.length === 0) {
        setProfile(profileDataCandidate.body);
        const profileSkillIdsCandidate = new Set(
          profileDataCandidate.body.skill_list.map(
            (skill: skillExp) => skill.id
          )
        );
        let skillsToChoose = skillResp.body.filter(
          (skill: skillDto) => !profileSkillIdsCandidate.has(skill.id)
        );
        setSkillsCandidate(skillsToChoose);
      }
    }
  }, [isSuccess]);
      ...
      if (profileData.messages.length === 0) {
        ...
        if (isSkillsReturnedSuccess && skillResp.messages.length === 0) {
          const profileSkillIds = new Set(
            profileData.body.skill_list.map((skill: skillExp) => skill.id)
          );
          let skillsToChoose = skillResp.body.filter(
            (skill: skillDto) => !profileSkillIds.has(skill.id)
          );
  ...
  useEffect(() => {
    if (
      !deleteProfileSkillResponse.isLoading &&
      (deleteProfileSkillResponse.isError ||
        deleteProfileSkillResponse.isSuccess) &&
      deleteProfileSkillResponse.data.messages.length != 0
    ) {
      setError(
        new ErrorResponse(true, deleteProfileSkillResponse.data.messages)
      );
    }
  }, [deleteProfileSkillResponse]);
  useEffect(() => {
    if (
      !putProfileResponse.isLoading &&
      (putProfileResponse.isError || putProfileResponse.isSuccess) &&
      putProfileResponse.data != undefined &&
      putProfileResponse.data.messages != undefined &&
      putProfileResponse.data.messages.length != 0
    ) {
  ...
  function removeErrorMsg(msg: Message) {
    setError({
      ...
      messages: error.messages.filter(
        (errMsg) => msg.message != errMsg.message
      ),
    });
  }
  ...
  function handleSaveProfile() {
    setProfile({
      ...profile,
      skill_list: [...profile.skill_list, ...newSkills],
    });
    let skillList = [...profile.skill_list, ...newSkills].filter(
      (s) => s.skill
    );
  ...
  function handleNewSkillDelete(index: number, skill: skillExp) {
    let updatedSkills: skillExp[] = [
      ...newSkills.slice(0, index),
      ...newSkills.slice(index + 1),
    ];
  ...
  function handleSkillAdd() {
    const s: skillExp = {
      name: skills[0].name,
      skill: skills[0].id,
      level_of_experience: experiences[0],
    };
  ...
  return (
    <>
      {error.messages.length > 0 &&
        error.messages.map((msg: Message, index: number) => (
          <Snackbar
            key={index}
            open={error.exist}
          ...
          >
            <Alert severity={msg.code === "ERROR" ? "error" : "success"}>
              {" "}
              {msg.message}
            </Alert>
          </Snackbar>
        ...
        <Box sx={style.profileContainer}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={3}>
              <Avatar sx={style.avatar} alt="Profile">
                {getFirstTwoInitialCharacters(profile.name)}
              </Avatar>
            </Grid>
            <Grid item xs={12} sm={3}>
              {editing && !isSuccess ? (
          ...
          <Divider>
            <Typography sx={style.dividerChip}> Skills</Typography>
          </Divider>
          <>
            {editing && !isSuccess ? (
              <>
                ...
                {newSkills.map((skill: skillExp, index: number) => (
                  <Box sx={style.editSkillBox} key={index}>
                    ...
                    <Button
                      ...
                      color="info"
                      startIcon={<DeleteIcon />}
                      onClick={() => handleNewSkillDelete(index, skill)}
                    ></Button>
                  </Box>
                ))}
              </>
